--comment all -- commit
-- move all from fac _id and cms_Security to TEST

 select * from
       all_constraints where
            r_constraint_name in
                 (select  constraint_name from all_constraints
                  where table_name= 'CMS_STG_FAC_BNM_CODES'); 
                  
                  
 select * from
       all_constraints where
            constraint_name IN ('FKF5FD22F08A6108AE', 'FK4D8592A28A6108AE');
...
create or replace PACKAGE HOUSE_KEEP AS
	...
	--COL
	PROCEDURE REPLICATE_TRANSACTION_COL_RELATED_REC(p_col_ids IN NUMBER_ARRAY);
	PROCEDURE DELETE_TRANSACTION_COL_RELATED_REC(p_col_ids IN NUMBER_ARRAY);
...
create or replace PACKAGE BODY "HOUSE_KEEP" AS
	...
	FROM sci_lsp_appr_lmts WHERE rownum <5000;
	-- WHERE CMS_LSP_APPR_LMTS_ID IN (20130606000000001,20130619000000025);

	SELECT CMS_COLLATERAL_ID BULK COLLECT INTO p_col_ids
	FROM CMS_SECURITY where rownum<5000;
	-- WHERE CMS_COLLATERAL_ID IN (20130606000000007, 20130606000000009) ;
	...
	REPLICATE_COL_RELATED_STG_REC(p_col_ids);
    DELETE_TRANSACTION_COL_RELATED_REC(p_col_ids);
    DELETE_COL_RELATED_REC(p_col_ids);
    DELETE_COL_RELATED_STG_REC(p_col_ids);
    ...
    p_fac_master_ids NUMBER_ARRAY;
	BEGIN
	DBMS_OUTPUT.PUT_LINE ('START DELETE_FAC_RELATED_REC');
    
    SELECT ID BULK COLLECT INTO p_fac_master_ids
    FROM CMS_FACILITY_MASTER 
	WHERE CMS_LSP_APPR_LMTS_ID IN ( SELECT * FROM TABLE(p_fac_ids) );

	DELETE FROM CMS_FAC_RELATIONSHIP
	WHERE CMS_FAC_MASTER_ID IN ( SELECT * FROM TABLE(p_fac_master_ids) );
	...
	INSERT INTO HK_CMS_STG_FAC_MULTI_TIER_FINANCI
	SELECT * FROM CMS_STG_FAC_MULTI_TIER_FINANCI 
	WHERE CMS_FAC_MASTER_ID IN ( SELECT * FROM TABLE(p_fac_ids) );
    ...
    p_fac_stg_master_ids NUMBER_ARRAY;
	BEGIN
	DBMS_OUTPUT.PUT_LINE ('START DELETE_FAC_RELATED_STG_REC');	

    
    SELECT ID BULK COLLECT INTO p_fac_stg_master_ids
    FROM CMS_STG_FACILITY_MASTER 
	WHERE ID IN ( SELECT * FROM TABLE(p_fac_ids) );

	DELETE FROM CMS_STG_FACILITY_MASTER 
	WHERE ID IN ( SELECT * FROM TABLE(p_fac_ids) );	

	DELETE FROM CMS_STG_FAC_RELATIONSHIP 
	WHERE CMS_FAC_MASTER_ID IN ( SELECT * FROM TABLE(p_fac_stg_master_ids) );

	DELETE FROM CMS_STG_FAC_MULTI_TIER_FINANCI
	WHERE CMS_FAC_MASTER_ID IN ( SELECT * FROM TABLE(p_fac_stg_master_ids) );
...
PROCEDURE DELETE_TRANSACTION_COL_RELATED_REC(p_col_ids IN NUMBER_ARRAY) AS
	...
	WHERE COLLATERAL_ID IN ( SELECT * FROM TABLE(p_col_ids)) ;
	...
	PROCEDURE REPLICATE_COL_RELATED_STG_REC(p_col_ids IN NUMBER_ARRAY) AS
	BEGIN 
	...
	PROCEDURE DELETE_COL_RELATED_REC(p_col_ids IN NUMBER_ARRAY) AS
	BEGIN 
	...
	--collateral_id
	DELETE FROM CMS_ASSET_TRADE_IN WHERE COLLATERAL_ID IN (SELECT * FROM TABLE(p_col_ids));
	...
	PROCEDURE DELETE_COL_RELATED_STG_REC(p_col_ids IN NUMBER_ARRAY) AS
	BEGIN 
	...
	DELETE FROM STAGE_ASSET_TRADE_IN WHERE COLLATERAL_ID IN (SELECT * FROM TABLE(p_col_ids));
...
-- 	SELECT COUNT(*) FROM sci_lsp_appr_lmts ; -- 10765
-- SELECT COUNT(*) FROM CMS_SECURITY ; -- 8247
-- START TRANSACTION;
