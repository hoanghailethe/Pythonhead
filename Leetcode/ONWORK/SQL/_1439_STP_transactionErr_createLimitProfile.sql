...
where transaction_type = 'COL'
    and reference_id in (
        ...
        from cms_security
        where security_no = 'CS202208110022639629'
    );
select *
from trans_history
...
select *
from cms_security
where security_no = 'AB202208160022658657';
select *
...
from transaction
where transaction_type = 'COL'
    and reference_id in (20220726020559087, 20220726020559085);
select *
from TRANS_HISTORY
where transaction_id in (
        select transaction_id
...
SELECT CMS_COLLATERAL_ID
FROM CMS_SECURITY
WHERE LOS_SECURITY_DTL_ID = 202205250022068591 ;
-- TO
select CMS_COLLATERAL_ID
from cms_ca_collateral_map map
where map.los_bca_ref_num LIKE '%CAR/22/9001/00200/05103744%' ;

--825
--ADD CHECKING TRANSATACTION INFORMATION
SELECT *
FROM STAGE_LIMIT_PROFILE
WHERE LOS_BCA_REF_NUM LIKE '%CAR/22/9001/00200/05103744%';
select *
from CMS_STAGE_CHARGE_DETAIL
where cms_collateral_id in (
        Select CMS_COLLATERAL_ID
...
SELECT *
FROM TRANSACTION o
WHERE o.LIMIT_PROFILE_REF_NUM like '%CAR/22/9001/00200/05103744%';
select *
from TRANS_HISTORY
where reference_id in (
        select transaction_id
...
WHERE transaction_type = 'AA_PROFILE'
    AND CUSTOMER_ID IN (
        ...
        WHERE LSP_LE_ID LIKE '%3010538592%'
    );
...
WHERE transaction_type = 'AA_PROFILE'
    AND reference_id IN (
        ...
        WHERE llp_bca_ref_num = '3010538592'
    );
select *
from TRANS_HISTORY
where transaction_id in (
...
SELECT lm.*
from sci_lsp_lmt_profile lm,
    CMS_CA_COLLATERAL_MAP map
where LM.CMS_LSP_LMT_PROFILE_ID = map.CMS_LSP_LMT_PROFILE_ID
    AND map.CMS_COLLATERAL_ID IN (
        select CMS_COLLATERAL_ID
        from cms_ca_collateral_map map
        where map.los_bca_ref_num LIKE '%CAR/22/9001/00200/05103744%'
    );
SELECT *
FROM CMS_ASSET
WHERE cms_asset.CMS_COLLATERAL_ID IN (
        select CMS_COLLATERAL_ID
        from cms_ca_collateral_map map
        where map.los_bca_ref_num LIKE '%CAR/22/9001/00200/05103744%'
    );
...
WHERE chrg.CMS_COLLATERAL_ID IN (
        select CMS_COLLATERAL_ID
        from cms_ca_collateral_map map
        where map.los_bca_ref_num LIKE '%CAR/22/9001/00200/05103744%'
    );
SELECT *
FROM CMS_FAC_RELATIONSHIP fac,
    CMS_CHARGE_DETAIL cd
WHERE fac.CMS_REF_ID = cd.CMS_REF_ID
    AND cd.CMS_COLLATERAL_ID IN (
        select CMS_COLLATERAL_ID
        from cms_ca_collateral_map map
        where map.los_bca_ref_num LIKE '%CAR/22/9001/00200/05103744%'
    );
...
FROM cms_facility_master fm
    LEFT JOIN CMS_FAC_RELATIONSHIP fac ON fm.ID = fac.CMS_FAC_MASTER_ID
    JOIN CMS_CHARGE_DETAIL cd ON fac.CMS_REF_ID = cd.CMS_REF_ID
    AND cd.CMS_COLLATERAL_ID IN (
        select CMS_COLLATERAL_ID
        from cms_ca_collateral_map map
        where map.los_bca_ref_num LIKE '%CAR/22/9001/00200/05103744%'
...
SELECT *
FROM CMS_LIMIT_CHARGE_MAP CM,
    CMS_CHARGE_DETAIL C
WHERE C.CMS_COLLATERAL_ID IN (
        select CMS_COLLATERAL_ID
        from cms_ca_collateral_map map
        where map.los_bca_ref_num LIKE '%CAR/22/9001/00200/05103744%'
    )
    AND C.CHARGE_DETAIL_ID = CM.CHARGE_DETAIL_ID;
SELECT *
FROM CMS_LIMIT_SECURITY_MAP lsm
WHERE lsm.CMS_COLLATERAL_ID IN (
        select CMS_COLLATERAL_ID
        from cms_ca_collateral_map map
        where map.los_bca_ref_num LIKE '%CAR/22/9001/00200/05103744%'
    );
...
WHERE SCI_PLEDGOR_DTL.CMS_PLEDGOR_DTL_ID = SCI_SEC_PLDGR_MAP.CMS_PLEDGOR_DTL_ID
    ...
    AND cms_security.CMS_COLLATERAL_ID IN (
        select CMS_COLLATERAL_ID
        from cms_ca_collateral_map map
        where map.los_bca_ref_num LIKE '%CAR/22/9001/00200/05103744%'
    );
...
SELECT llp_bca_ref_num,
    sci_lsp_aa_profile.*
FROM sci_lsp_aa_profile
WHERE llp_le_id = '3010538592' --CIF                              
